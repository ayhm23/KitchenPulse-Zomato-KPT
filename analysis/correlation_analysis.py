...
FIX: Chart 1 signal dict corrected 'raw_for_kpt' → 'raw_kpt'
     (signal_denoiser.py produces 'raw_kpt', not 'raw_for_kpt').
"""
...
def chart_correlation_heatmap(df):
    ...
    signals = {
        ...
        'Raw FOR KPT'             : 'raw_kpt',            # FIX: was 'raw_for_kpt'
        'Corrected FOR KPT'       : 'corrected_for_kpt',  # FIX: was 'corrected_kpt'
        'POS KPT'                 : 'pos_kpt',
    ...
    ax.set_xticklabels(corr_df.columns, rotation=45, ha='right', fontsize=8, color=SUBTEXT)
...
def chart_for_bias_distribution(df):
    ...
    axes[0].set_title('FOR Button Delay Distribution', color=TEXT, fontsize=11, fontweight='bold')
    ...
    axes[1].set_xticklabels(['Honest\nMerchants', 'Biased\nMerchants'], color=SUBTEXT, fontsize=9)
    axes[1].set_ylabel('Delay (minutes)', color=SUBTEXT)
    axes[1].set_title('FOR Button Delay — Box Summary', color=TEXT, fontsize=11, fontweight='bold')
    axes[1].axhline(0, color='white', linestyle='--', linewidth=1, alpha=0.4)

    med_h = np.median(honest_min)
    med_b = np.median(biased_min)
    axes[1].text(1, med_h + 0.3, f'Median: {med_h:.1f}m', ha='center', color=GREEN, fontsize=8, fontweight='bold')
    axes[1].text(2, med_b + 0.3, f'Median: {med_b:.1f}m', ha='center', color=RED, fontsize=8, fontweight='bold')
...
# ══════════════════════════════════════════════════════════════════════════════
# CHART 3 — Hidden Load vs Rider Wait
# ══════════════════════════════════════════════════════════════════════════════
def chart_hidden_load_impact(df):
    ...
    z = np.polyfit(sample['hidden_load'], sample['actual_rider_wait_minutes'].clip(0, 20), 1)
    xr = np.linspace(sample['hidden_load'].min(), sample['hidden_load'].max(), 100)
    axes[0].plot(xr, np.poly1d(z)(xr), color='white', linewidth=2, linestyle='--', alpha=0.8)
    corr = df['hidden_load'].corr(df['actual_rider_wait_minutes'])
    axes[0].set_title(f'Hidden Load vs Rider Wait  (r = {corr:.3f})', color=TEXT, fontsize=11, fontweight='bold')
    ...
    bars = axes[1].bar(avg_wait.index, avg_wait.values, color=colors_q, width=0.5, edgecolor='none')
    axes[1].set_title('Avg Rider Wait by Hidden Load Quartile', color=TEXT, fontsize=11, fontweight='bold')
    ...
    for bar, v in zip(bars, avg_wait.values):
        axes[1].text(bar.get_x() + bar.get_width() / 2, v + 0.05,
                     f'{v:.2f}m', ha='center', va='bottom', color='white', fontsize=9, fontweight='bold')
...
# ══════════════════════════════════════════════════════════════════════════════
# CHART 4 — Hourly Kitchen Load Heatmap
# ══════════════════════════════════════════════════════════════════════════════
def chart_hourly_kli_heatmap(df):
    ...
    im = ax.imshow(pivot.values, cmap='RdYlGn_r', aspect='auto', vmin=20, vmax=80)
...
def chart_tier_improvement(df):
    ...
    for tier in ['T1', 'T2', 'T3']:
        ...
        if len(sub) < 10:
            continue
        bm = mae_fn(sub['naive_kpt_estimate'], sub['true_kpt_minutes'])
        km = mae_fn(sub['kli_adjusted_kpt'], sub['true_kpt_minutes'])
        tiers.append(tier)
        base_maes.append(bm)
        kp_maes.append(km)
    ...
    axes[0].set_title('KPT Error by Merchant Tier', color=TEXT, fontsize=11, fontweight='bold')
    axes[0].legend(facecolor=PANEL, edgecolor=GRID, labelcolor=TEXT, fontsize=9)
    ...
    for bar, v in zip(b1, base_maes):
        axes[0].text(bar.get_x() + bar.get_width() / 2, v + 0.05,
                     f'{v:.2f}m', ha='center', va='bottom', color=RED, fontsize=8, fontweight='bold')
    for bar, v in zip(b2, kp_maes):
        axes[0].text(bar.get_x() + bar.get_width() / 2, v + 0.05,
                     f'{v:.2f}m', ha='center', va='bottom', color=GREEN, fontsize=8, fontweight='bold')

    colors_imp = [GREEN if i > 0 else RED for i in improvements]
    bars = axes[1].bar(tiers, improvements, color=colors_imp, width=0.4, edgecolor='none', alpha=0.85)
    axes[1].set_ylabel('MAE Improvement (%)', color=SUBTEXT)
    axes[1].set_title('% Improvement by Tier', color=TEXT, fontsize=11, fontweight='bold')
...
# ══════════════════════════════════════════════════════════════════════════════
# CHART 6 — Signal Accuracy Ladder
# ══════════════════════════════════════════════════════════════════════════════
def chart_signal_accuracy_ladder(df):
    ...
    signal_map = {
        ...
        'Raw FOR Button'                   : 'raw_kpt',            # FIX: was 'raw_for_kpt'
        'Corrected FOR Button'             : 'corrected_for_kpt',  # FIX: was 'corrected_kpt'
    ...
    bars = ax.barh(labels, maes, color=colors, alpha=0.85, height=0.5, edgecolor='none')
    ax.set_xlabel('Mean Absolute Error vs True KPT (minutes)', color=SUBTEXT)
    ax.set_title('Signal Accuracy Ladder — Lower is Better', color=TEXT, fontsize=12, fontweight='bold')
    ...
    for bar, v in zip(bars, maes):
        ax.text(v + 0.05, bar.get_y() + bar.get_height() / 2,
                f'{v:.2f} min', va='center', color='white', fontsize=9, fontweight='bold')
...
def print_pdf_stats(df):
    ...
    if all(c in df.columns for c in ['naive_kpt_estimate', 'kli_adjusted_kpt', 'true_kpt_minutes']):
        base_mae = mae_fn(df['naive_kpt_estimate'], df['true_kpt_minutes'])
        kp_mae = mae_fn(df['kli_adjusted_kpt'], df['true_kpt_minutes'])
    ...
    if 'pos_kpt' in df.columns and 'raw_kpt' in df.columns:
        pos_mae = mae_fn(df['pos_kpt'], df['true_kpt_minutes'])
        raw_mae = mae_fn(df['raw_kpt'], df['true_kpt_minutes'])
